...
| Split full-run state | **2026-09-12 (part 12 + bugfix 2-4):** ForestPack eval `split.sh 30000 BrushifyForestPack --windowed --cameras` **RUNNING** with the mission console as the `--cameras` panel. Stack ALIVE: split 2257244, server 2257497, tunnel 2260184, viewer **2450609** (manually relaunched after fixes — kill separately via `/tmp/aerovla_cameras.pid`, NOT the split pid). H100 eval pid 39888 ALIVE, `Completed: 14 / 438`, no manual flag → not blocked. Viewer window 1200x800 at `+1894+167` on `:1`, log 0 errors, all views live + **UI freeze fixed** (`ee54608`: beacon ssh moved off the tkinter main thread — was freezing the whole window ~6s every 2s; now FRONT repaints ~0.75s, telemetry live 40/40). **Manual mode VERIFIED (2026-09-12):** H100 `{"manual": true}` → eval blocked at Step 114 for 40s (0 new steps); `{"manual": false}` → resumed (Step 116, drone moved). Only untested link = the actual M keypress (no xdotool to inject); the M→flag code writes the identical verified format. **Gotchas: window moves on each relaunch — re-read `xwininfo` before cropping screenshots.** Next: real keypress test optional; monitor eval. |
